...
from pinit_api.utils.string_operations import *


class TestComputeInitialFromEmail(TestCase):
    def test_compute_initial_from_email_with_letter(self):
        email = "john.doe23@example.com"
        computed_initial = compute_initial_from_email(email)
        self.assertEqual(computed_initial, "J")

    def test_compute_initial_from_email_without_letter(self):
...
class TestComputeUsernameCandidateFromEmail(TestCase):
    def test_compute_username_candidate_from_email_with_alphabetical_characters(self):
        email = "john.doe23@example.com"
        computed_candidate = compute_username_candidate_from_email(email)
        self.assertEqual(computed_candidate, "johndoe")

    def test_compute_username_candidate_from_email_without_alphabetical_character(self):
        email = "1234@example.com"
        computed_candidate = compute_username_candidate_from_email(email)
...
class TestComputeFirstAndLastNameFromEmail(TestCase):
    def test_compute_first_and_last_name_from_email_happy_case(self):
        ...
        (
            computed_first_name,
            computed_last_name,
        ) = compute_first_and_last_name_from_email(email)
        self.assertEqual(computed_first_name, "John")
        self.assertEqual(computed_last_name, "Doe")

    def test_compute_first_and_last_name_from_email_no_separator(self):
    ...
    def test_compute_first_and_last_name_from_email_no_letters(self):
