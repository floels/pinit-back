...
class AuthenticationTests(APITestCase):
    ...
    def test_obtain_jw_token_wrong_credentials(self):
        ...
        response_wrong_username = self.client.post('/api/token/', data_wrong_username, format='json')

        self.assertEqual(response_wrong_username.status_code, status.HTTP_401_UNAUTHORIZED)

        data_wrong_password = { 'username': self.user_username, 'password': 'somePa$$word'}
        response_wrong_password = self.client.post('/api/token/', data_wrong_password, format='json')
    ...
    def test_refresh_jw_token_wrong_refresh(self):
        ...
        """
        response = self.client.post('/api/token/refresh/', {'refresh': 'wrong.refreshToken'}, format='json')
